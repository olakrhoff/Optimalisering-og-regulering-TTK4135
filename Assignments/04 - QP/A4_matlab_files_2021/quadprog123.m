function [X,fval,exitflag,output,lambda]=quadprog123(H,f,A,B,Aeq,Beq,lb,ub,X0,options)
...
if isequal(output.algorithm, medium)
   ...
   [X,lambdaqp,exitflag,output]= ...
      qpsub123(full(H),f,[full(Aeq);full(A)],[Beq;B],lb,ub,X0,neqcstr,verbosity,caller,ncstr,numberOfVariables,options); 
   output.algorithm = medium;
...
elseif isequal(output.algorithm,large)  % largescale: call sqpmin when just bounds or just equalities
   ...
   if exitflag == -2  % Problem not handled by sqpmin at this time
      ...
      [X,lambdaqp,exitflag,output]= ...
         qpsub123(full(H),f,[full(Aeq);full(A)],[Beq;B],lb,ub,X0,neqcstr,verbosity,caller,ncstr,numberOfVariables);
      output.algorithm = medium;
   end
end
