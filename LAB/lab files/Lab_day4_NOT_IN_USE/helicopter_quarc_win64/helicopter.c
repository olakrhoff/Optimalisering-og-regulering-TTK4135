 ...
 * C source code generated on : Thu Feb  2 13:54:00 2023
 *
...
void helicopter_output(void)
{
   ...
   */
  temp_tmp = helicopter_M->Timing.t[0];
  rtb_Backgain = helicopter_P.SquareWaveGenerator_Frequency * temp_tmp;
  if (rtb_Backgain - floor(rtb_Backgain) >= 0.5) {
    /* SignalGenerator: '<Root>/Square Wave Generator' */
    helicopter_B.p_ref = helicopter_P.SquareWaveGenerator_Amplitude;
  } else {
    /* SignalGenerator: '<Root>/Square Wave Generator' */
    helicopter_B.p_ref = -helicopter_P.SquareWaveGenerator_Amplitude;
  }
   ...
   *  Sum: '<S7>/Sum3'
   */
  rtb_Clock = ((helicopter_B.p_ref - helicopter_P.Gain1_Gain_f *
                helicopter_B.Gain) * helicopter_P.K_pp -
   ...
   *  Gain: '<S2>/Gain1'
   */
  rtb_Derivative = helicopter_B.elev_ref - helicopter_P.Gain1_Gain_f *
    helicopter_B.elev;
  ...
  rtb_Backgain = ((helicopter_P.K_ep * rtb_Derivative +
                   helicopter_X.Integrator_CSTATE) - helicopter_P.Gain1_Gain_f *
  ...
  /* Derivative: '<S6>/Derivative' */
  if ((helicopter_DW.TimeStampA >= temp_tmp) && (helicopter_DW.TimeStampB >=
       temp_tmp)) {
    rtb_Derivative = 0.0;
  } else {
    ...
    if (helicopter_DW.TimeStampA < helicopter_DW.TimeStampB) {
      if (helicopter_DW.TimeStampB < temp_tmp) {
        rtb_Derivative = helicopter_DW.TimeStampB;
        lastU = &helicopter_DW.LastUAtTimeB;
      }
    } else {
      if (helicopter_DW.TimeStampA >= temp_tmp) {
        rtb_Derivative = helicopter_DW.TimeStampB;
        lastU = &helicopter_DW.LastUAtTimeB;
      }
    }

    rtb_Derivative = (helicopter_B.PitchCounttorad - *lastU) / (temp_tmp -
      rtb_Derivative);
  }
   ...
   *  Sum: '<S1>/Subtract'
   */
  temp_tmp = (rtb_Backgain - rtb_Clock) * helicopter_P.Backgain_Gain;

  /* Saturate: '<S6>/Back motor: Saturation' */
  if (temp_tmp > helicopter_P.BackmotorSaturation_UpperSat) {
    /* Saturate: '<S6>/Back motor: Saturation' */
    helicopter_B.BackmotorSaturation = helicopter_P.BackmotorSaturation_UpperSat;
  } else if (temp_tmp < helicopter_P.BackmotorSaturation_LowerSat) {
    /* Saturate: '<S6>/Back motor: Saturation' */
    helicopter_B.BackmotorSaturation = helicopter_P.BackmotorSaturation_LowerSat;
  } else {
    /* Saturate: '<S6>/Back motor: Saturation' */
    helicopter_B.BackmotorSaturation = temp_tmp;
  }
   ...
   *  Sum: '<S1>/Add'
   */
  temp_tmp = (rtb_Clock + rtb_Backgain) * helicopter_P.Frontgain_Gain;

  /* Saturate: '<S6>/Front motor: Saturation' */
  if (temp_tmp > helicopter_P.FrontmotorSaturation_UpperSat) {
    /* Saturate: '<S6>/Front motor: Saturation' */
    helicopter_B.FrontmotorSaturation =
      helicopter_P.FrontmotorSaturation_UpperSat;
  } else if (temp_tmp < helicopter_P.FrontmotorSaturation_LowerSat) {
    /* Saturate: '<S6>/Front motor: Saturation' */
    helicopter_B.FrontmotorSaturation =
      helicopter_P.FrontmotorSaturation_LowerSat;
  } else {
    /* Saturate: '<S6>/Front motor: Saturation' */
    helicopter_B.FrontmotorSaturation = temp_tmp;
  }
...
RT_MODEL_helicopter_T *helicopter(void)
{
  ...
  /* External mode info */
  helicopter_M->Sizes.checksums[0] = (2045961349U);
  helicopter_M->Sizes.checksums[1] = (3511020980U);
  helicopter_M->Sizes.checksums[2] = (880873744U);
  helicopter_M->Sizes.checksums[3] = (2911727797U);
